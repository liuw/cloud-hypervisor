...
pub struct PerformanceTestOverrides {
    ...
    pub test_image_format: Option<ImageFormat>,
}
...
pub struct ListedPerformanceTest {
    pub name: &'static str,
    pub control: String,
}
...
) -> Result<PerformanceTestResult, RunError> {
    let (sender, receiver) = channel::<Result<PerformanceTestResult, RunError>>();
...
fn matching_tests(test_filter: &[&str], test_exclude: &[&str]) -> Vec<&'static PerformanceTest> {
    TEST_LIST
        .iter()
        .filter(|test| !(cfg!(target_arch = "aarch64") && test.name == "virtio_net_latency_us"))
        .filter(|test| {
            test_filter.is_empty() || test_filter.iter().any(|filter| test.name.contains(filter))
        })
        .filter(|test| {
            !test_exclude
                .iter()
                .any(|exclude| test.name.contains(exclude))
        })
        .collect()
}

fn execute_tests(
    tests_to_run: Vec<&'static PerformanceTest>,
    overrides: &PerformanceTestOverrides,
) -> Result<MetricsReport, RunError> {
    ...
    let needs_vm_tests = tests_to_run
        .iter()
        .any(|test| !test.name.starts_with("micro_"));

    if needs_vm_tests {
        init_tests(overrides.as_ref());
    }
    ...
    for test in tests_to_run {
        ...
        metrics_report
            .results
            .push(run_test_with_timeout(test, &overrides)?);
    }
...
pub fn listed_tests(test_filter: &[&str], test_exclude: &[&str]) -> Vec<ListedPerformanceTest> {
    matching_tests(test_filter, test_exclude)
        .into_iter()
        .map(|test| ListedPerformanceTest {
            name: test.name,
...
pub fn run_test_by_name(
    test_name: &str,
    overrides: &PerformanceTestOverrides,
) -> Result<PerformanceTestResult, RunError> {
    let test = matching_tests(&[], &[])
        .into_iter()
        .find(|candidate| candidate.name == test_name)
        .ok_or_else(|| RunError::UnknownTest(test_name.to_owned()))?;

    execute_tests(vec![test], overrides)?
        .results
        .into_iter()
        .next()
        .ok_or_else(|| RunError::UnknownTest(test_name.to_owned()))
...
pub fn run_tests(
    ...
    execute_tests(matching_tests(test_filter, test_exclude), overrides)
}

pub fn write_report<W: Write>(writer: &mut W, metrics_report: &MetricsReport) -> io::Result<()> {
    writer.write_all(
...
pub fn run_cli() {
    ...
    let test_exclude: Vec<&str> = cmd_arguments
        .get_many::<String>("test-exclude")
        .map(|values| values.map(String::as_str).collect())
        .unwrap_or_default();

    if cmd_arguments.get_flag("list-tests") {
        for test in listed_tests(&test_filter, &test_exclude) {
    ...
    let overrides = PerformanceTestOverrides {
    ...
    let metrics_report = run_tests(&test_filter, &test_exclude, &overrides).unwrap_or_else(|e| {
