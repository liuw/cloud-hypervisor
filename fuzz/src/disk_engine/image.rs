...
/// writes, about 5 us.
///
/// The cache is keyed by format name: a fuzz target only ever materializes
/// one template, but the self tests exercise several in one process.
fn template_pages(name: &'static str, bytes: &[u8]) -> Arc<Vec<usize>> {
    /// Cached page maps, one per format name.
    type PageMaps = HashMap<&'static str, Arc<Vec<usize>>>;
...
pub fn template_memfd(name: &'static str, bytes: &[u8]) -> io::Result<File> {
    let pages = template_pages(name, bytes);
    let mut file = memfd(name)?;
...
pub fn template_file(name: &'static str, bytes: &[u8]) -> io::Result<(File, PathBuf)> {
    let pages = template_pages(name, bytes);
    let (mut file, path) = scratch_image(name)?;
...
mod tests {
    ...
    fn template_materialization_reproduces_the_buffer() {
        ...
        assert_eq!(*template_pages("sparse-test", &bytes), vec![0, 2, 3]);

        // An empty buffer and a wholly zero buffer are degenerate cases of
        // the same loop, and each needs its own cache key.
        for (i, len) in [0usize, 1, SPARSE_PAGE, SPARSE_PAGE + 1].iter().enumerate() {
            let zeroes = vec![0u8; *len];
            let name: &'static str = Box::leak(format!("zero-test-{i}").into_boxed_str());
            let mut file = template_memfd(name, &zeroes).expect("memfd");
            let mut read_back = Vec::new();
            file.read_to_end(&mut read_back).expect("read back");
